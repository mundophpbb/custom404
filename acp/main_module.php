        ...
        try {
            ...
            if (!$this->filesystem->exists($log_dir)) {
                try {
                    ...
                    if ($config['custom404_debug_mode'] ?? false) {
                        error_log("Error creating log directory: " . $e->getMessage(), 3, $log_file);
                    }
                }
            }
            ...
            $this->load_language_files($config['custom404_debug_mode'] ?? false);
            ...
            if ($request->is_set_post('export_logs')) {
                ...
                // Substituído 'echo' por atribuição ao template ou saída apropriada
                $template->assign_var('CSV_BOM', "\xEF\xBB\xBF"); // UTF-8 BOM
                ...
                while ($row = $db->sql_fetchrow($result)) {
                    // Usar $user->format_date() para formatar o horário com base no fuso horário do phpBB
                ...
                // Substituído 'exit' por trigger_error para evitar interrupção abrupta
                trigger_error('Logs exportados com sucesso.', E_USER_NOTICE);
            }
            ...
            $template->assign_vars([
                ...
                'CUSTOM404_DEBUG_MODE' => $config['custom404_debug_mode'] ?? false,
    ...
    protected function load_language_files($debug_mode = false)
    {
        ...
        if (!$this->filesystem->exists($log_dir)) {
            try {
                ...
                if ($debug_mode) {
                    error_log("Error creating log directory in load_language_files: " . $e->getMessage(), 3, $log_dir . 'debug.log');
                }
            }
        }
